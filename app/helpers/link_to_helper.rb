...
module LinkToHelper
  def link_to_new(resource, url)
    ...
    link_to(body, url, title: title, class: 'btn btn-sm btn-primary')
  end
  ...
  def link_to_edit(url)
    link_to_default(:edit, url, class: 'btn btn-sm btn-success')
  end

  def link_to_back(url)
    link_to_default(:back, url, class: 'btn btn-secondary')
  end

  def link_to_show(url)
    link_to_default(:show, url, class: 'btn btn-sm btn-info')
  end

  def link_to_destroy(url, opt = {})
    link_to_default(:destroy, url,
                    class: "btn btn-sm btn-danger #{opt[:class]}",
                    method: :delete,
